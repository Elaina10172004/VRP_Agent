...
def best_intra_route_two_opt(
    ...
    current_score = score_cvrptw_routes(routes, distance_matrix, demands, capacity, time_windows, service_times, objective_spec)
    ...
    for route_index, route in enumerate(routes):
        ...
        for i in range(len(route) - 1):
            for j in range(i + 1, len(route)):
                ...
                candidate_score = score_cvrptw_routes(candidate_routes, distance_matrix, demands, capacity, time_windows, service_times, objective_spec)
    ...
    for from_route_index, from_route in enumerate(routes):
        for from_pos, customer in enumerate(from_route):
            ...
            for to_route_index in target_route_indices:
                ...
                for insert_pos in candidate_insert_positions(to_route, [customer], neighbor_lists):
    ...
    for from_route_index, from_route in enumerate(routes):
        for segment_length in segment_lengths:
            ...
            for start in range(len(from_route) - segment_length + 1):
                ...
                for to_route_index in target_route_indices:
                    to_route = routes[to_route_index]
                    for insert_pos in candidate_insert_positions(to_route, segment, neighbor_lists):
                        updated = [list(route) for route in routes]
...
def _collect_customer_insertions(
    ...
    for route_index in target_route_indices:
        ...
        for insert_pos in candidate_insert_positions(route, [customer], neighbor_lists):
            ...
            candidate_routes = clean_routes(updated)
            candidate_score = score_cvrptw_routes(
                candidate_routes,
                distance_matrix,
                demands,
                capacity,
...
def _shaw_related_customers(
    ...
    seed_ready, seed_due = time_windows[seed_customer]
    related = []
    for customer in route_of:
        node_index = customer + 1
        ready, due = time_windows[customer]
        score = (
            distance_matrix[seed_node][node_index]
            + 0.2 * abs(demands[seed_customer] - demands[customer])
            + 0.05 * (abs(seed_ready - ready) + abs(seed_due - due))
        )
...
def best_shaw_regret_move(
    ...
    for route in routes:
        for position, customer in enumerate(route):
            prev_node = 0 if position == 0 else route[position - 1] + 1
            next_node = 0 if position == len(route) - 1 else route[position + 1] + 1
            ...
            seed_candidates.append((break_cost, customer))
    seed_candidates.sort(reverse=True)
    seed_pool = [customer for _cost, customer in seed_candidates[: min(4, len(seed_candidates))]]
    if not seed_pool:
        seed_pool = flattened[:1]

    for seed_customer in seed_pool:
        removal_order = _shaw_related_customers(routes, distance_matrix, demands, time_windows, seed_customer)
...
def best_route_elimination(
    ...
    for route_index in route_order:
        ...
        candidate_score = score_cvrptw_routes(working_routes, distance_matrix, demands, capacity, time_windows, service_times, objective_spec)
        if is_better_score(candidate_score, best_score):
            best_score = candidate_score
            best_routes = working_routes
...
def reduce_cvrptw_vehicle_count(
    ...
    current_score = score_cvrptw_routes(current_routes, distance_matrix, demands, capacity, time_windows, service_times, objective)
    neighbor_lists = _neighbor_lists(distance_matrix, config)
    applied = 0

    for _ in range(max_rounds):
        move = best_route_elimination(
            current_routes,
            distance_matrix,
            demands,
            capacity,
...
def improve_cvrptw_solution(
    ...
    for iteration in range(max_rounds):
        ...
        for operator in operators:
            if operator == "two_opt":
                move = best_intra_route_two_opt(current_routes, distance_matrix, demands, capacity, time_windows, service_times, objective)
                ...
                move = best_swap_move(current_routes, distance_matrix, demands, capacity, time_windows, service_times, objective)
            elif operator == "or_opt":
                move = best_or_opt_move(
                    current_routes,
                    distance_matrix,
                    demands,
                    capacity,
                ...
                move = best_two_opt_star_move(current_routes, distance_matrix, demands, capacity, time_windows, service_times, objective)
            elif operator == "cross_exchange":
                move = best_cross_exchange_move(
                    current_routes,
                    distance_matrix,
                    demands,
                    capacity,
